// Most of the functionality of this library is based on the SL25 API
...
void SL25::setAddress(uint8_t new_addr)
{
...
const char * SL25::rangeStatusToString(RangeStatus status)
{
    switch (status) {
        case RangeValid:
            return "range valid";
...
void SL25::readResults()
{
    ...
    results.dss_actual_effective_spads_sd0  = (uint16_t)value[3] << 8; // high byte
    results.dss_actual_effective_spads_sd0 |=           value[4];      // low byte

//    Wire.read(); // peak_signal_count_rate_mcps_sd0: not used
//    Wire.read();

    results.ambient_count_rate_mcps_sd0  = (uint16_t)value[7] << 8; // high byte
    results.ambient_count_rate_mcps_sd0 |=           value[8];      // low byte
...
//    Wire.read();
    ...
    results.final_crosstalk_corrected_range_mm_sd0  = (uint16_t)value[13] << 8; // high byte
    results.final_crosstalk_corrected_range_mm_sd0 |=           value[14];      // low byte

    results.peak_signal_count_rate_crosstalk_corrected_mcps_sd0  = (uint16_t)value[15] << 8; // high byte
    results.peak_signal_count_rate_crosstalk_corrected_mcps_sd0 |=           value[16];      // low byte
}
